...
$sheetId = null;
$selectedType = null;
$fieldDefs = ['fields' => [], 'result_label' => 'نتیجه', 'result_unit' => null];
...
if ($selectedType) {
    $fieldDefs = lab_get_internal_test_fields($testTypeId);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $rawData = [];
        foreach ($fieldDefs['fields'] as $f) {
            $rawData[$f['key']] = trim($_POST['raw'][$f['key']] ?? '');
        }

        if ($sampleId <= 0) {
            $errors[] = 'نمونه را انتخاب کنید.';
        }
        // The foam sheet (test type 5) has no single mandatory result;
        // every other sheet requires the final result field.
        if ($resultValue === '' && $testTypeId !== 5) {
            $errors[] = 'مقدار ' . $fieldDefs['result_label'] . ' را وارد کنید.';
        }
        ...
        if ($testedDateJ !== '') {
            ...
            $testedDateG = date('Y-m-d');
        }

        if (!$errors) {
            $stmt = $pdo->prepare(
                "INSERT INTO test_results
                    (sample_id, internal_log_sheet_id, result_value, raw_data, tested_date)
                 VALUES
                    (:sample_id, :internal_log_sheet_id, :result_value, :raw_data, :tested_date)"
            );
            $stmt->execute([
                ...
                'result_value'          => $resultValue !== '' ? $resultValue : null,
...
<head>
    ...
    <style>
        ...
        .card { background:#fff; border-radius:10px; padding:24px; max-width:900px; margin:0 auto 24px; box-shadow:0 1px 4px rgba(0,0,0,.08); }
        ...
        .result-field { background:#fff8e1; border:1px solid #ffe082; border-radius:6px; padding:12px; margin-top:16px; }
        .raw-details { font-size:11px; color:#777; max-width:260px; }
        ...
        <div class="card">
            ...
            <form method="post">
                <div class="grid">
                    ...
                    <?php foreach ($fieldDefs['fields'] as $f): ?>
                        <div>
                            <label>
                                <?= htmlspecialchars($f['label']) ?>
                                <?php if ($f['unit']): ?><span class="optional">(<?= htmlspecialchars($f['unit']) ?>)</span><?php endif; ?>
                            </label>
                            <input type="text" name="raw[<?= htmlspecialchars($f['key']) ?>]">
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="result-field">
                    <label style="margin-top:0;">
                        <b><?= htmlspecialchars($fieldDefs['result_label']) ?></b>
                        <?php if ($fieldDefs['result_unit']): ?><span class="optional">(<?= htmlspecialchars($fieldDefs['result_unit']) ?>)</span><?php endif; ?>
                    </label>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th><?= htmlspecialchars($fieldDefs['result_label']) ?></th>
                        <th>تاریخ آزمایش</th>
                        <th>جزئیات خام</th>
                        <th>وضعیت انتقال</th>
                    </tr>
                ...
                <tbody>
                    <?php foreach ($results as $r): ?>
                        <tr>
                            ...
                            <td><?= $r['result_value'] !== null ? htmlspecialchars($r['result_value']) : '—' ?></td>
                            <td><?= htmlspecialchars($r['tested_date_fa']) ?></td>
                            <td class="raw-details">
                                <?php
                                    $parts = [];
                                    foreach (($r['raw_data_decoded'] ?? []) as $k => $v) {
                                        if ($v === '' || $v === null) continue;
                                        $labelMatch = '';
                                        foreach ($fieldDefs['fields'] as $f) {
                                            if ($f['key'] === $k) { $labelMatch = $f['label']; break; }
                                        }
                                        $parts[] = ($labelMatch ?: $k) . ': ' . $v;
                                    }
                    ...
                    <?php endforeach; ?>
                    <?php if (!$results): ?>
                        <tr><td colspan="6" style="text-align:center;color:#999;">هنوز نتیجه‌ای ثبت نشده است.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
            </div>
